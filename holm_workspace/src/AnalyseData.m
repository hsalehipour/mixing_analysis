...
%     Ri_bul(i) = mean(N2(shl_ind,i),1)./ (mean(shear(shl_ind,i),1).^2);
% end
...
%% 2-11 ingredients of sigma2d and sigma3d
ind_sig2d = time<=t2d;
ind_sig3d = time<=t3d & time>=t2d;


avgH3d(ic)   = mean(sig3_Hb(ind_sig3d));
avgAn(ic)    = mean(sig3_An(ind_sig3d));
avgShbar(ic) = mean(sig3_Rs(ind_sig3d));
avgSh2d(ic)  = mean(sig3_Sh(ind_sig3d));
avgD3d(ic)   =-mean(sig3_Dv(ind_sig3d));
avgsig3d(ic) = mean(sigma3d(ind_sig3d));

%avgXP(ic)   = mean(XP(ind_sig3d));
%avgM(ic)    = mean(M(ind_sig3d));
%avgD(ic)    =-mean(D(ind_sig3d));
%avgS(ic)    = mean(S(ind_sig3d));
%avgDp(ic)   = mean(time(ind_sig3d)*0+Dp);
%avgXK(ic)   = mean(XK(ind_sig3d));
%avgH2d(ic)   = mean(H2d(ind_sig3d));

%avgD2d(ic)   = -mean(D2d(ind_sig3d));
...
gradrho  = ddx(rhobar,z);
kappa_cox  = mean(chi_pr)'  ./mean(gradrho.^2)';
% kappa_cox could also be defined based on chi_3d = |grad(t_3d)|^2
% here subscript 'pr' means perturbation from mean (i.e. 2d+3d)
...
%       overestimates N.
%       So I can re-calculate the averaging of all parameters over the
...
%% 2-17 calculate Surrgate for M and turb dissipation epsilon'(z)
% a surrogate for mixing M
M_sur = chi_pr./abs(gradrhob)*Ri/R/Re/Pr - Dp*Lz;

% eps' = eps_tot-eps_mean
eps_mean = shear.^2./Re;
eps_prt = epsbar - eps_mean;